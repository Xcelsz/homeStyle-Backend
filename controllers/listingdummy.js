...
exports.createListing = async (req, res) => {
  try {
    ...
    const tableDetails = {
      ...
      Property: {
        tableName: 'PropertyDetails',
        columns: `category, PropertyNeed, title, subtitle, price, displayImage, serviceSummary, generalInfo, features, propertyAmenities, location, payment_options, status, service_details, service_type, size, bedRoom, parking, bathRoom, area, videoLinks, faq, propertyUsage, total`,
        values: [
          category,
          PropertyNeed,
          title,
...
exports.getAllServiceListings = async (req, res) => {
  try {
    const [result] = await pool.query(
      "SELECT * FROM ServiceDetails ORDER BY service_details_id DESC"
    );
...
exports.getAllServiceListings = async (req, res) => {
  try {
    const [result] = await pool.query(
      "SELECT * FROM ServiceDetails WHERE status = 'published' ORDER BY service_details_id DESC"
    );
